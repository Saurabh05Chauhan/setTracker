...
export class AnalysisComponent implements OnInit {
 ...
 EarlierData:any[]=[];
  ...
  ngOnInit(): void {
    ...
    this.sTservice.GetExerciseData().valueChanges().subscribe((res:any)=>{
      if(res.length>0){
        this.recordData=res;
...
this.EarlierData.sort(sortByProperty('date', 'desc'))
  ...
  onSubmit(){
    ...
    this.RecordForm.controls['date'].patchValue(this.datepipe.transform((new Date), 'MM-dd-yyyy')?.toString())
